...
<body>
	...
	<table>
		...
		<c:if test="${dto.photo_name ne null}">
			<tr>
				<th>사진</th>
				<td colspan="3"><img width="500" src="/photo/${dto.photo_name}" />
					<input type="hidden" name="photo_id" value="${dto.photo_id}" /></td>
			</tr>
		</c:if>
		<tr>
			<th colspan="4">
				<div>
					<input type="button" onclick="location.href='freeList.do'"
						value="리스트" />
				</div>
				<div>
					<input type="button"
						onclick="location.href='./freeUpdate.go?board_id=${dto.board_id}'"
						value="수정" />
				</div>
				<div>
					<input type="button"
						onclick="location.href='./freeDelete.go?board_id=${dto.board_id}'"
						value="삭제" />
				</div>
			</th>
		</tr>
		...
		<c:forEach items="${commentList}" var="commentList" varStatus="status">
			<tr class="reply">
				...
				<td>
					<p class="readMode">${commentList.comment_content}</p> <textarea
						rows="3" name="comment_content" class="editMode none">${commentList.comment_content}</textarea>
				</td>
		...
		<form action="freecommentWrite.do" method="post">
			<input type="hidden" name="board_id" value="${dto.board_id}">
			<th><input type="hidden" name="user_id"
				value="<%=(String) session.getAttribute("loginId")%>" /> <b><%=(String) session.getAttribute("loginId")%></b>
			</th>
			<th><input type="text" name="comment_content" /></th>
			<th colspan="2">
				<button>작성</button>
			</th>
		</form>
